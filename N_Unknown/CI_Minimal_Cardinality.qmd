...
```{r}
Sys.time()
mst_ac_N_unknown(M = 25, m = 15, conf_level = 0.95, max_N = 50)
Sys.time()
mst_ac_N_unknown_vec(M = 25, m = 15, conf_level = 0.95, max_N = 50)
Sys.time()
mst_ac_N_unknown_direct(M = 25, m = 15, conf_level = 0.95, max_N = 50)
Sys.time()
```
...
bk_ac_N_unknown_vec <- function(M, m, conf_level = 0.95, max_N = 1000) {
  ...
  for (current_N in M:max_N) {
    ...
    # For the BK procedure, order in increasing order of a and b.
...
```{r}
Sys.time()
test<- bk_ac_N_unknown(M = 25, m = 15, conf_level = 0.95, max_N = 205)
test
Sys.time()
```

```{r}
bk <- bk_ac_N_unknown(M = 25, m = 15, conf_level = 0.95, max_N = 50)
```
...
cg_ac_N_unknown_vec <- function(M, m, conf_level = 0.95, max_N = 1000) {
  ...
  for (current_N in M:max_N) {
    ...
    # For CG, we order by descending a and descending b.
    setorder(subset_dt, -a, -b)
    chosen_row <- subset_dt[1]
...
```{r}
Sys.time()
test<- cg_ac_N_unknown(M = 25, m = 15, conf_level = 0.95, max_N = 205)
test
Sys.time()
```
...
```{r}
minimal_cardinality_ci_N_unkown_vec(M = 10, m = 5, conf_level = 0.95, max_N = 100, 
                                procedure = "MST")
```
