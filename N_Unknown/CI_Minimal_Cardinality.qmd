...
```{r}
# Plot of All Minimal Cardinality Acceptance Curves (with trends shown pt 2)

# Initialize an empty data frame to store results
filtered_plot2 <- data.frame()

# Loop through each row in all_ac_mc_M_10_m_4
for (i in 1:nrow(all_ac_mc_M_10_m_4)) {
  # Get the current N and x_set
  current_N <- all_ac_mc_M_10_m_4$N[i]
  current_x_set <- all_ac_mc_M_10_m_4$x_set[i]
  ...
  filtered_plot2 <- bind_rows(filtered_plot, filtered_rows)
}

# Remove any duplicate rows just in case
filtered_plot2 <- distinct(filtered_plot)


conf_level= 0.95

ggplot(filtered_plot2, aes(x = N, 
                 y = coverage_prob, 
                 group = x_set, 
                 color = x_set)) +
  geom_line() +
