...
all_ac_N_unknown_vec <- function(M, m, conf_level = 0.95, max_N = 1000) {
  ...
  # (1) Build triangular data frame of (a,b) with a <= b
  ab_df <- do.call(
    rbind,
    lapply(0:max_x, function(a_val) {
      data.frame(a = a_val, b = a_val:max_x)
    })
  )
  
  # (2) Combine with N in [M..max_N]
  N_seq <- seq.int(M, max_N)
  df_full <- data.frame(
    N = rep(N_seq, each = nrow(ab_df)),
    a = rep(ab_df$a, times = length(N_seq)),
    b = rep(ab_df$b, times = length(N_seq))
  )
  
  # (3) Vectorized coverage
  coverage_vec <- mapply(
  ...
  cardinality_vec <- df_full$b - df_full$a + 1
  
  # (4) Add columns
  df_full$coverage_prob <- coverage_vec
  df_full$cardinality   <- cardinality_vec
  
  # (5) Filter coverage >= conf_level & coverage in [0,1]
  df_filter <- subset(
    df_full,
    coverage_prob >= conf_level &
      coverage_prob <= 1 &
  ...
  # (6) Add x_set column
  df_filter$x_set <- paste(df_filter$a, df_filter$b, sep = "-")
...
```{r}
test <- mst_ac_N_unknown(M = 25, m = 15, conf_level = 0.95, max_N = 49)
test
```
