...
relative_df <- expected_value_plot_df %>%
  pivot_wider(names_from = Method, values_from = ExpectedLength) %>%
  mutate(across(-c(N, M, m, CMC), ~ .x / CMC)) %>%
  pivot_longer(cols = -c(N, M, m, CMC), names_to = "Method", values_to = "RelativeExpectedLength") %>%
...
ggplot(relative_df, aes(x = N, y = RelativeExpectedLength, 
  ...
  labs(x = "N", y = "Relative Expected Length (to CMC)", 
       color = "Method", linetype = "Method") +
  theme_minimal() +
  theme(
    legend.position = "bottom",
...
# Rename method for cleaner legend
average_length_cp_plot_df$Method <- gsub(
  "Analog to Clopper-Pearson", "ACP", average_length_cp_plot_df$Method
)
...
# Plot
ggplot(average_length_cp_plot_df, aes(
  x = AverageLength, 
  y = AverageCoverageProbability, 
  color = Method, 
  fill = Method,             # Map fill to Method
  shape = Method             # Shapes 21–25 support fill
)) +
  geom_point(size = 3, stroke = 1.2) +  # stroke controls border thickness
  geom_hline(yintercept = 0.95, color = "red", linetype = "dashed") +
  facet_wrap(~ facet_label, scales = "free_x") +
  scale_y_continuous(limits = c(0.949, 0.973)) +
  scale_color_manual(values = distinct_colors) +
  scale_fill_manual(values = distinct_colors) +  # match fill to color
  scale_shape_manual(values = c(21, 22, 23, 24, 25, 21)) +  # pick from 21–25
  labs(
    x = "Average Length", 
