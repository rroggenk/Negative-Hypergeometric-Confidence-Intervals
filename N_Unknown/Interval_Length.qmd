...
```{r}
# expected_length <- function(data_frame, lb_col_name, ub_col_name, N, M, m, max_N = 250) {
#   max_x = max_N - M
#   sum = 0
#   for (xi in 0:max_x) {
...
expected_length <- function(data_frame, lb_col_name, ub_col_name, N, M, m, max_N = 250) {
  sum = 0
  for (xi in data_frame$x) {  # Loop through all x values in data_frame$x
    length = data_frame[[ub_col_name]][data_frame$x == xi] -
             data_frame[[lb_col_name]][data_frame$x == xi] + 1
    prob = ngh_pmf(x = xi, N = N, M = M, m = m)
    indiv = length * prob
...
## First choose M, m, and confidence level!
...
```{r}
data = CI_Analog_CP_N_Unknown(M = M, m = m, conf_level = conf_level, max_N = max_N)
average_length(data, "lower_bound", "upper_bound")

# Create a data frame to store the results
results = data.frame(N = integer(), ExpectedLength = numeric())

# Calculate expected length for each N
for (N_i in M:max_N) {
  exp_len = expected_length(data, "lower_bound", "upper_bound", N = N_i, M = M, m = m, 
                            max_N = max_N)
  results = rbind(results, data.frame(N = N_i, ExpectedLength = exp_len))
}

# Plot using ggplot2
ggplot(results, aes(x = N, y = ExpectedLength)) +
  geom_point() +
  geom_hline(yintercept = average_length(data, "lower_bound", "upper_bound"), color = "red") +
  labs(x = "N", y = "Expected Length") +
  theme_minimal()
```
...
```{r}
data = minimal_cardinality_ci_N_unkown_vec(M = M, m = m, conf_level = conf_level, 
