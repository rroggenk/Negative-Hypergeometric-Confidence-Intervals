...
```{r}
# Not working 100% yet, making sure a and b are non decreasing still not working 
...
all_mc_ac <- function(N, m, conf_level = 0.95) {
  ...
  # Filter out the sets with the smallest cardinality and coverage probability >= conf_level for M >= m
  filtered_results_M_ge_m = results_M_ge_m %>%
  ...
  filtered_results = bind_rows(filtered_results_M_ge_m, results_M_lt_m) %>%
    arrange(desc(M)) %>%
    select(-valid)
...
```{r}
test = all_mc_ac(N = 50, m = 3, conf_level = 0.95)
test 

# a and b non decreasing not working at M = 10, 0-21 inlcuded when it should not be 
```
