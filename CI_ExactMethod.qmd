...
blaker_ac <- function(N, m, conf_level = 0.95) {
  ...
  for (fixed_M in unique(results$M)) {
    if (fixed_M >= m) {
      acceptance_set = c()
      for (fixed_x in 0:N) {
        ...
        prob_sum = sum(sapply(possible_xs, function(px) ngh_pmf(x = px, 
                                                                N = N, 
                                                                M = fixed_M, 
                                                                m = m)))
      ...
      results = results %>% 
        mutate(acceptance_set = ifelse(M == fixed_M, acceptance_set_str, acceptance_set))
    }
  }
  
  # Keep only the unique combinations of M and acceptance_set
  final_results = results %>% 
    select(M, acceptance_set) %>% 
    distinct()
