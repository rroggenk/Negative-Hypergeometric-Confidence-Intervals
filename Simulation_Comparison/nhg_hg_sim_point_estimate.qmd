...
1.  **NHG Performs Better ((**Diff_MSE_N \> 0)):
    -   NHG tends to have lower MSE (positive differences) for (N) when:
        -   (M / N) (ratio of successes to population size) is **low** (e.g., below 0.3). This occurs when the population contains a small fraction of successes, and NHG captures sparsity better.
        -   (m / M) (ratio of observed successes to total successes) is **low** (e.g., below 0.2). Smaller observed samples tend to favor NHG.
    -   The minimum (M / N = 0.025) and (m / M = 0.028) show NHG excels in extremely sparse populations.
2.  **HG Performs Better ((**Diff_MSE_N \< 0)):
    -   HG performs better for (N) when:
        -   (M / N) is **high** (e.g., above 0.5). This suggests a denser population of successes, where HG benefits from its fixed sample size.
        -   (m / M) is **high** (e.g., above 0.4). Larger observed samples reduce variability, which HG leverages effectively.

#### **Performance of NHG vs. HG for (M)**:

1.  **NHG Performs Better ((**Diff_MSE_M \> 0)):
    -   NHG has lower MSE (positive differences) for (M) when:
        -   The ratio (m / M) is **low** (e.g., below 0.2). Smaller observed successes relative to total successes favor NHG’s fixed success framework.
        -   This aligns with cases where (N) is large and (M / N) is small, making the population sparse.
2.  **HG Performs Better ((**Diff_MSE_M \< 0)):
    -   HG performs better for (M) when:
        -   The ratio (m / M) approaches or exceeds 0.5. This means a significant portion of the successes in the population are observed, and HG benefits from the fixed sample size.
        -   (M / N) is moderate to high (e.g., above 0.3), suggesting denser populations.

#### **General Trends**:

-   **Impact of (M / N)**:
    -   NHG performs better when (M / N) is small, suggesting NHG is more effective for sparse populations.
    -   HG outperforms NHG when (M / N) increases, especially as it approaches 1.
-   **Impact of (m / M)**:
    -   NHG has an advantage for lower values of (m / M) (below 0.2), where fewer successes are observed.
    -   HG tends to excel as (m / M) increases, especially when (m / M \> 0.4).

### Summary Interpretation

-   **NHG excels** in situations where:
    -   The population is sparse ((M / N) is low).
    -   The number of observed successes is small relative to the total successes ((m / M) is low).
-   **HG excels** when:
    -   The population is dense ((M / N) is high).
    -   The number of observed successes is large relative to the total successes ((m / M) is high).
...
# Plot 2: Diff_MSE_N vs. m/M
ggplot(combined_results_2, aes(x = m_over_M, y = Diff_MSE_N, color = as.factor(N))) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "loess", se = FALSE) +
  labs(
    title = "Difference in MSE (N): NHG vs. HG by m/M",
...
# Plot 4: Diff_MSE_M vs. m/M
ggplot(combined_results_2, aes(x = m_over_M, y = Diff_MSE_M, color = as.factor(N))) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "loess", se = FALSE) +
  labs(
    title = "Difference in MSE (M): NHG vs. HG by m/M",
